...
export default component$(() => {
  const store = useStore<{ isScrolling: boolean; lang: string }>({
    isScrolling: false,
    lang: "en",
  });

  // Sync current language after i18n is initialized on client
  useVisibleTask$(() => {
    if (i18n.language) {
        ...
        <div class="hidden md:self-center md:flex items-center md:mb-0 fixed w-full md:w-auto md:static justify-end left-0 rtl:left-auto rtl:right-0 bottom-0 p-3 md:p-0">
          <div class="items-center flex justify-between w-full md:w-auto">
            ...
            <div class="ml-4">
              ...
              <select
                id="lang-select"
                class="border rounded px-2 py-1 bg-white dark:bg-slate-800 dark:border-slate-700"
                value={store.lang}
                onChange$={(ev, el) => {
                  const next = (el as HTMLSelectElement).value;
                  store.lang = next;
